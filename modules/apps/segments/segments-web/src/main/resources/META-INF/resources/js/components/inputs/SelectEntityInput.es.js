...
class SelectEntityInput extends React.Component {
	...
	_handleSelectEntity = () => {
		...
		if (multiple) {
			...
			openSelectionModal({
				onSelect: (event) => {
					console.log("SelectEntityInput.es.js onSelect Event:");
					console.log(event);
					if (event && event.entityid && event.entityname) {
						onChange({
						...
						if (event) {
							console.log("ERROR: Invalid event received:");
							console.log(event);
							console.log("Hack prevention of the modal dismissing itself");
							event.preventDefault();
						}
						else {
							console.log("ERROR: Empty event received.");
						}
